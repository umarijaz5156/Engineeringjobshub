...
class JobCreateService
{
    ...
    public function execute($request): Job
    {
        ...
        if ($request->ispost_councilDirect === 'true') {
            $this->sendJobToSecondWebsite($jobCreated,$request->categories);
        }

        if ($request->ispost_waterland === 'true') {
            $this->sendJobToWaterLand($jobCreated,$request->categories);
        }
    ...
    protected function sendJobToWaterLand($job,$categories)
    {
        $client = new Client();
        $categoryId = $categories[0] ?? 3;
         $websiteUrl = env('WEBSITE_URL_JOB_WATER_LAND');  
