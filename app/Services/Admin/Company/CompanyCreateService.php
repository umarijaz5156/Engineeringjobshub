...
class CompanyCreateService
{
    ...
    public function execute($request): void
    {
        ...
        if($request->CouncilDirectCompany === 'on'){
            try {
        ...
        if($request->WaterLandCompany === 'on'){
            try {
                $this->WaterLandCompany($request);
            } catch (\Exception $e) {
    ...
    private function WaterLandCompany($request) {

        $client = new Client();
        $url = env('WEBSITE_URL_COMPANY_WATER_LAND');  
